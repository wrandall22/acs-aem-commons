...
public class ScrMetadataIT {
    ...
    private List<String> compareDescriptors(Descriptor current, Descriptor latestRelease) {
        ...
        current.properties.stream().filter(cp -> !PROPERTIES_TO_IGNORE.contains(cp.name)).forEach(cp -> {
        ...
        latestRelease.properties.stream().filter(lp -> !PROPERTIES_TO_IGNORE.contains(lp.name)).forEach(lp -> {
